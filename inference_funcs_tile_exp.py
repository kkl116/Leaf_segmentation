...
def Run_Leaf_Inference(leaf_fpath, clust_by, number_of_classes, time_series_analysis, use_t0_as_reference, overlap_ids, out_dir, bright_norm,t,t0_fpath, leaf_learner_path = 'Leaf_UNet/v1', size = 64,  stats_test = 'anova'):
    ...
    #tile images first to reduce the amount of compression?
    slice_image(leaf_fpath, rescale_path)
    #need to make sure dims are even 
    fn_noext = img_fname[:-4]
    slice_paths = glob.glob(rescale_path+'/'+fn_noext+'*')
    rescaled_slices = [image_rescale(image = s, height = 512, width = 512, mode = "best") for s in slice_paths]
    for rs,sp in zip(rescaled_slices,slice_paths):
        cv2.imwrite(sp, rs)

    leaves_img = cv2.imread(img_path)
    for sp in slice_paths:
        img_slice = open_image(sp)
        pred = Leaf_inference(leaf_learner_path, img_slice)
        pred_np = np.uint8(pred[1].squeeze())
        pred_np = image_rescale(image = pred_np, height = leaves_img.shape[0], width = leaves_img.shape[1], mode = "best")
        pred_fname = 'm_' + os.path.split(sp)[1]
        cv2.imwrite(os.path.join(leaves_dir,pred_fname), pred_np)
    #join the slices back tgt
    tiles = image_slicer.open_images_in(leaves_dir)
    mask = image_slicer.join(tiles)
    mask.save('joined_slices.png')
    ...
    #inference
    if time_series_analysis != 'y' or time_series_analysis == 'y' and t == 0 or time_series_analysis == 'y' and t != 0 and use_t0_as_reference == 'n':
...
def compile_time_series_results(time_series_analysis, ts_res,out_dir, ts_dir, missing_tps, impute_method):
    ...
    for measure in range(4):
        ...
        for group in range(len(ts_res)):
            ...
            measure_df = measure_df.append(m_dat)
        mdf_fname = m_names[measure] + '.xlsx'
        mdf_fpath = os.path.join(ts_path, mdf_fname)
        measure_df.to_excel(mdf_fpath)
